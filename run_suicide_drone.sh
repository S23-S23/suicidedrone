...
source ~/ros2_ws/suicidedrone/install/setup.bash
 ros2 launch follower_drone follower_drone.launch.py drone_id:=2 "$@"
 
 # 3. 현재 날짜/시간으로 폴더 및 파일명 생성
LOG_DIR="/home/suvlab/suicidedrone_log"
mkdir -p $LOG_DIR
DATE_STR=$(date +%Y_%m_%d_%H_%M_%S)
BAG_NAME="$LOG_DIR/rosbag_$DATE_STR"

# 4. rosbag 기록 시작
echo "3. rosbag 기록 시작: $BAG_NAME"
