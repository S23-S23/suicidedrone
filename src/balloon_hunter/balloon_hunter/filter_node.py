#!/usr/bin/env python3
"""
Filter Node — DKF / EKF / GT pixel estimation (full 18-state, paper-faithful)
================================================================
...
Parameter 'filter_type': 'DKF' (full 18-state DKF with delay replay, default),
                         'EKF' (same 18-state model, delay_steps=0), or 'GT'.
"""
...
class FilterNode(Node):
    def __init__(self):
        ...
        if self.filter_type == 'GT':
            ...
            raise ValueError(
                f"Unknown filter_type '{self.filter_type}'. Expected 'DKF', 'EKF', or 'GT'."
            )
        ...
        self._max_predict_age = 1.0          # seconds without measurement before freezing
    ...
    def predict_and_publish(self):
        ...
        if self.filter_type == 'GT':
            ...
            if self._predict_frozen:
                return  # stop publishing stale estimates after max_predict_age
